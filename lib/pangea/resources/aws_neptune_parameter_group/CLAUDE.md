# AwsNeptuneParameterGroup Implementation Documentation

## Overview

This directory contains the implementation for the `aws_neptune_parameter_group` resource function, providing type-safe creation and management of Neptune Parameter Group resources through terraform-synthesizer integration.
...
The main `aws_neptune_parameter_group` function that:
...
NeptuneParameterGroupAttributes dry-struct defining:
...
```ruby
class NeptuneParameterGroupAttributes < Dry::Struct
...
```ruby
resource(:aws_neptune_parameter_group, name) do
...
RSpec.describe Pangea::Resources::AWS do
  describe "#aws_neptune_parameter_group" do
