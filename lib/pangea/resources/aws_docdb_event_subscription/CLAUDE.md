# AwsDocdbEventSubscription Implementation Documentation

## Overview

This directory contains the implementation for the `aws_docdb_event_subscription` resource function, providing type-safe creation and management of Docdb Event Subscription resources through terraform-synthesizer integration.
...
The main `aws_docdb_event_subscription` function that:
...
DocdbEventSubscriptionAttributes dry-struct defining:
...
```ruby
class DocdbEventSubscriptionAttributes < Dry::Struct
...
```ruby
resource(:aws_docdb_event_subscription, name) do
...
RSpec.describe Pangea::Resources::AWS do
  describe "#aws_docdb_event_subscription" do
