    ...
    module UI
      ...
      class Diff
        ...
        def terraform_plan(plan_output)
          ...
          sections = Diff::PlanParser.parse_plan_output(plan_output)
        ...
        def display_resource_section(section)
          icon, color = Diff::Formatting.action_style(section[:action])
        ...
        def display_changed_line(content)
          if content =~ /(.+) = (.+) -> (.+)$/
            ...
            formatted = Diff::Formatting.format_value_change(old_val, new_val, @pastel)
        ...
        def display_attribute_change(attribute, change)
          ...
          if change[:old] && change[:new]
            ...
            puts "  #{@pastel.green('+')} #{attribute}: #{Diff::Formatting.format_value(change[:add])}"
          elsif change[:remove]
            puts "  #{@pastel.red('-')} #{attribute}: #{Diff::Formatting.format_value(change[:remove])}"
          end
        end

        def display_modified_attribute(attribute, change)
          old_val = Diff::Formatting.format_value(change[:old])
          new_val = Diff::Formatting.format_value(change[:new])
          formatted = Diff::Formatting.format_value_change(old_val, new_val, @pastel)
