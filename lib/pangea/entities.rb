...
# Entities are provided by pangea-core when available.
# This file provides fallback definitions for standalone operation.
...
module Pangea
  module Entities
    ...
    # Define Namespace only if not already provided by pangea-core
    unless const_defined?(:Namespace)
      class Namespace < Dry::Struct
        class StateConfig < Dry::Struct
          ...
          def validate_s3!
            ...
            lock_table_val = dynamodb_table || lock
            errors << "DynamoDB lock table is required" if lock_table_val.nil?
        ...
        class State < Dry::Struct
          ...
          def self.new(attributes)
            ...
            if state_type == :s3
              ...
              missing_fields = required_fields.select { |field| config_attrs[field].nil? || config_attrs[field].to_s.empty? }
    ...
    # Define CompilationResult only if not already provided by pangea-core
    unless const_defined?(:CompilationResult)
